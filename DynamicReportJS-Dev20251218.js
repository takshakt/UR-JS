...
console.log('globalAttrCols:>>>>>>>>>>>>>',globalAttrCols);
  ...
  SELECT hotel_id, ${[
        ...cols,
         ...
         ROW_NUMBER() OVER (PARTITION BY hotel_id ORDER BY ${orderCol}) rn_id
  FROM ${table}
  ...
  SELECT 
      ...
      ROW_NUMBER() OVER (PARTITION BY a.id ORDER BY STAY_DATE) rn_id
  FROM (
 ...
 ROW_NUMBER() OVER (PARTITION BY id ORDER BY STAY_DATE) rn_id
 from UR_HOTEL_PRICE_OVERRIDE
  ...
  SELECT
      ...
      t.STAY_DATE as pk_col,
      t.attribute_value,
      ROW_NUMBER() OVER (ORDER BY t.STAY_DATE) as rn_id
  FROM TABLE(
...
const joinClauses = allAliases.slice(1).map((alias, i) => {
  ...
  const joinType = (alias.startsWith('s') || alias.startsWith('ga')) ? ' LEFT ' : 'FULL OUTER';
...
FROM ${allAliases[0]}
${joinClauses.join("\n")}
ORDER BY COALESCE(${allAliases.map(a => a + ".rn_id").join(", ")}, 0)`;
...
function addtablcolsformula(configData){

    console.log('before adding in tableColumns');

// Loop through all formulas in configData
Object.entries(configData.formulas).forEach(([calcName, formulaObj]) => {
    const ctype = formulaObj.type; // e.g., 'number', 'string'

    // Prepare new column metadata object
    ...
    if (!tableColumns.find(col => col.name === calcName)) {
        console.log('inside adding in tableColumns', newCalcColumn);
        ...
        // Add to selectedColumns with proper metadata
        jsondata_details.selectedColumns.push({
            col_name: calcName,
            temp_name: 'calc',
            alias_name: calcName,
...
console.log('after adding in tableColumns', tableColumns);
...
function loadConfigFromJSON(configData) {
    ...
    //console.log('loadConfigFromJSON call:>>>',configData);
    addtablcolsformula(configData);
...
function applySavedFilter(clearFlag = false) {
    return new Promise((resolve, reject) => {
        ...
        const filteredRows = originalReportData.filter(row => {
            let executableFilter = convertDatesInStringReversible(validatedFilterTemplate);
...
function convertDatesInStringReversible(input) {
      input = String(input ?? '');
...
function applyAggregations() {
    ...
    function addToFormula() {
        ...
        if (operator.includes('Day')){
                formulaPreview.value += `${operator}`+'('+`${column}`+')';
        }else{
        if (column) {
    ...
    if (conditionalExpression && (bracketedCheck.test(conditionalExpression) || plainCheck.test(conditionalExpression))) {
        conditionalExpression = replaceColumnOccurrences(conditionalExpression, fullColName, colType, rowValue, 'filter');
           ...
           if (ctype === 'date') {
                ...
                row[calcName] = result === null || result === undefined ? 'Calculation Issue' : String(result);
            }

        });

            console.log('before adding in tablcols');
        // --- Add metadata column if not present ---
        const newCalcColumn = { name: calcName, type: ctype };
        if (!tableColumns.find(col => col.name === calcName)) {
            console.log('inside adding in tablcols',newCalcColumn);
            tableColumns.push(newCalcColumn);
            report_expressions.columnMetadata.push(newCalcColumn);
            savedCalculationColumns.push(newCalcColumn);
        ...
        if (operator) { 
            if (operator === 'Range'){
               // formulaPreview.value += ` between ${document.getElementById('range-from').value} and ${document.getElementById('range-to').value} ` ;
                formulaPreview.value += ` between ${formatDate(document.getElementById('range-from').value)} 
                                        and ${formatDate(document.getElementById('range-to').value)} ` ;
