...
function createCard(index, data) {
    ...
    // 7. GROUP BY LOGIC - Find date column with aggregation
    ...
    if (groupByColumn && groupByAlias) {
        ...
        groupedData = Object.keys(groups).map(groupKey => {
            ...
            // Also process formula columns that might not be in selectedColumns but are in columns
            columns.forEach(col => {
                const alias = col.name || col.key;
                if (aggregatedRow[alias] === undefined && tempNameMap[alias] === 'calc') {
                    // This is a formula column that needs aggregation
                    const values = groupRows.map(row => {
                        let value = row[alias];
                        if (value === undefined || value === null) return 0;
                        if (typeof value === 'string') {
    ...
    // 10. HTML Generation with Sticky Footer
