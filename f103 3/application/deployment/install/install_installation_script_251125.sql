prompt --application/deployment/install/install_installation_script_251125
begin
--   Manifest
--     INSTALL: INSTALL-Installation Script 251125
--   Manifest End
...
wwv_flow_imp_shared.create_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_install_id=>wwv_flow_imp.id(9665506831982728)
,p_name=>'Installation Script 251125'
,p_sequence=>50
...
'    -- ============================================================================',
unistr('    -- \2728 UPDATED: Added configurable matching parameters'),
'    --',
...
'    FOR i IN 1 .. v_col_cnt LOOP',
'      IF v_json_obj.has(v_columns(i).column_name) THEN',
'        IF v_pk_cols.EXISTS(1) AND v_pk_cols.exists(i) AND v_pk'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'_pk_cols(i) = v_columns(i).column_name THEN',
...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
's IS NOT NULL THEN l_vals := l_vals || '', ''; END IF;',
'            l_vals := l_vals || ''p.''||l_mapping(k).parser_col||'' AS "''||l_mapping(k).tgt_col||''"'';',
'',
'            -- UPDATE list (skip key columns)',
'            IF l_mapping(k).tgt_col NOT IN (''HOTEL_ID'',''STAY_DATE'') THEN',
...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'ame_json := v_json_new.equals(v_json_old);',
'    EXCEPTION',
'    WHEN OTHERS THEN',
'        v_same_json := FALSE;',
'    END;',
...
'',
'        ----------------------------------------------------------------------',
unistr('        -- (3) Formula validation \2192 every referenced field must be NUMBER type'),
'        ----------------------------------------------------------------------',
...
unistr('        -- \2705 Replace this block with FN_CLEAN_NUMBER'),
...
'  ''      THEN TO_DATE(TRIM(p.''|| l_mapping(k).parser_col ||''), ''''DD-MON-YYYY'''') ''||',
'',
unistr('  -- \2705 NEW: Just date DD-MON-RR (2-digit year)'),
...
unistr('    -- \D83D\DFE2 Use MERGE for UPSERT when STAY_DATE qualifier exists'),
'    l_stay_val := TO_CHAR(fn_safe_to_date(l_stay_val), ''DD/MM/YYYY'');',
'',
'    l_sql_main :=',
'        ''MERGE INTO ''|| l_table_name ||'' t',
...
'             VALUES (s.HOTEL_ID, ''||rtrim(l_col_s, '', '')||'',s.INTERFACE_LOG_ID)'';',
'ELSE',
unistr('    -- \D83D\DD35 No STAY_DATE in template \2192 simple INSERT only'),
'    l_sql_main :=',
'        ''INSERT INTO ''|| l_table_name ||'' (HOTEL_ID,''|| l_cols ||'',INTERFACE_LOG_ID)',
'         SELECT ''''''||p_hotel_id||'''''' AS HOTEL_ID,''|| l_cols ||'',''''''||l_log_id||'''''' AS INTERFACE_LOG_ID ''||',
'        ''  FROM ( SELECT  ''||l_vals_calculation||'' FROM ( SELECT '' || rtrim(l_sql_select, '', '')  ||'' FROM DUAL)p''|| ',
...
unistr('    ELSE ''Warning'' END||'': Upload completed \2192 Total='' || l_total_rows || '', Success='' || l_success_cnt || '', Failed='' || l_fail_cnt;'),
'*/',
'p_message :=',
'    CASE',
'        WHEN l_total_rows = l_success_cnt THEN',
unistr('            ''Success: Upload completed \2192 Total='' || l_total_rows ||'),
...
'',
'        ELSE',
unistr('            ''Warning: Upload completed \2192 Total='' || l_total_rows ||'),
'            '', Success='' || l_success_cnt ||',
'            '', Failed='' || l_fail_cnt',
'    END;',
'',
...
'',
'                    -- Capture STAY_DATE value',
'                     INSERT INTO debug_log(message) VALUES(l_stay_col_name||''--- check stay_date'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
'             VALUES (s.HOTEL_ID, ''||rtrim(l_col_s, '', '')||'',s.INTERFACE_LOG_ID)'';',
'ELSE',
unistr('    --  No STAY_DATE in template \2192 simple INSERT only'),
'    l_sql_main :=',
'        ''INSERT INTO ''|| l_table_name ||'' (HOTEL_ID,''|| l_cols ||'',INTERFACE_LOG_ID)',
'         SELECT ''''''||p_hotel_id||'''''' AS HOTEL_ID,''|| l_cols ||'',''''''||l_log_id||'''''' AS INTERFACE_LOG_ID ''||',
'        ''  FROM ( SELECT  ''||l_vals_calculation||'' FROM ( SELECT '' || rtrim(l_sql_select, '', '')  ||'' FROM DUAL)p''|| ',
...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'O debug_log(message) VALUES(''--- l_sql_main:>   ''||l_sql_main);',
'        ',
'            ',
'                EXECUTE IMMEDIATE l_sql_main;',
'           ',
...
'        IF rec.pr_template IS NOT NULL THEN l_pr_attr_id := get_attribute_id_from_template(REPLACE(rec.pr_template, ''#'', '''')); IF l_pr_attr_id IS NOT NULL AND NOT l_aliases.EXISTS(l_pr_attr_id) THEN l_aliases(l_pr_attr_id) := l_pr_attr_id; END IF; EN'
||'D IF;',
'        -- Only extract attributes from expression if it''s NOT free text (not wrapped in ~text~)',
'        IF NOT REGEXP_LIKE(rec.expr, ''^~.*~$'') THEN',
'          LOOP l_attr_in_expr := REGEXP_SUBSTR(rec.expr, ''#[A-F0-9]+#'', l_pos); EXIT WHEN l_attr_in_expr IS NULL; l_clean_attr := REGEXP_REPLACE(l_attr_in_expr, ''^#|#$'', ''''); IF NOT l_aliases.EXISTS(l_clean_attr) THEN l_aliases(l_clean_attr) := l_cle'
||'an_attr; END IF; l_pos := INSTR(rec.expr, l_attr_in_expr, l_pos) + LENGTH(l_attr_in_expr); END LOOP;',
'        END IF;',
'      END;',
'    END LOOP;',
'',
...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
' l_occ_attr_id VARCHAR2(255); l_pr_template_id VARCHAR2(255); l_pr_attr_id VARCHAR2(255);',
...
'                        || ''}''',
'                        || '', "occupancy":'' || CASE WHEN cond_rec.occ_attr IS NOT NULL THEN ''{"attr":"'' || REPLACE(cond_rec.occ_attr,''#'','''') || ''", "actual":'' || NVL(TO_CHAR(l_actual_occ_val),''null'') || '', "operator":"'' || cond_rec.occ_op || ''", "thr'
||'eshold":'' || TO_CHAR(cond_rec.occ_val) || '', "failed":'' || CASE WHEN l_occ_failed THEN ''true'' ELSE ''false'' END || ''}'' ELSE ''null'' END',
'                        || '', "property_ranking":'' || CASE WHEN cond_rec.pr_type IS NOT NULL THEN ''{"attr":"'' || NVL(l_pr_attr_id,'''') || ''", "actual":'' || NVL(TO_CHAR(l_actual_pr_val),''null'') || '', "operator":"'' || cond_rec.pr_op || ''", "threshold":'''
||' || TO_CHAR(cond_rec.pr_val) || '', "failed":'' || CASE WHEN l_pr_failed THEN ''true'' ELSE ''false'' END || ''}'' ELSE ''null'' END',
'                        || '', "event_score":'' || CASE WHEN cond_rec.es_val IS NOT NULL THEN ''{"actual":'' || NVL(TO_CHAR(l_actual_event_score),''null'') || '', "operator":"'' || cond_rec.es_op || ''", "threshold":'' || TO_CHAR(cond_rec.es_val) || '', "failed'
||'":'' || CASE WHEN l_ev_failed THEN ''true'' ELSE ''false'' END || ''}'' ELSE ''null'' END',
...
'                      LOOP l_attr_in_expr := REGEXP_SUBSTR(cond_rec.expr, ''#[A-F0-9]+#'', 1, l_pos); EXIT WHEN l_attr_in_expr IS NULL; l_current_id := REGEXP_REPLACE(l_attr_in_expr, ''^#|#$'', ''''); l_attr_val := TO_CHAR(get_value_for_date(l_staged_data('
||'l_current_id), v_stay_date)); l_eval_copy := REPLACE(l_eval_copy, ''#'' || l_current_id || ''#'', NVL(l_attr_val, ''0'')); IF l_attr_counter > 0 THEN l_attr_json := l_attr_json || '',''; END IF; l_attr_json := l_attr_json || ''"'' || l_current_id || ''":'' || NV'
||'L(l_attr_val, ''null''); l_pos := l_pos + 1; l_attr_counter := l_attr_counter + 1; END LOOP;',
...
||'|| ''"stay_window":"'' || TO_CHAR(region_rec.stay_from,''YYYY-MM-DD'') || '' to '' || TO_CHAR(region_rec.stay_to,''YYYY-MM-DD'') || ''"'' || '', "lead_time":'' || l_lead_time_json || CASE WHEN region_rec.days_of_week IS NOT NULL THEN '', "days_of_week":'' || REPLA'
||'CE(region_rec.days_of_week,''"'',''\"'') ELSE '''' END || CASE WHEN region_rec.minimum_rate IS NOT NULL THEN '', "minimum_rate":'' || TO_CHAR(region_rec.minimum_rate) ELSE '''' END || ''}'' || '', "occupancy":'' || CASE WHEN cond_rec.occ_attr IS NOT NULL THEN ''{"a'
||'ttr":"'' || REPLACE(cond_rec.occ_attr,''#'','''') || ''", "actual":'' || NVL(TO_CHAR(l_actual_occ_val),''null'') || '', "operator":"'' || cond_rec.occ_op || ''", "threshold":'' || TO_CHAR(cond_rec.occ_val) || '', "failed":'' || CASE WHEN l_occ_failed THEN ''true'' EL'
||'SE ''false'' END || ''}'' ELSE ''null'' END || '', "property_ranking":'' || CASE WHEN cond_rec.pr_type IS NOT NULL THEN ''{"attr":"'' || NVL(l_pr_attr_id,'''') || ''", "actual":'' || NVL(TO_CHAR(l_actual_pr_val),''null'') || '', "operator":"'' || cond_rec.pr_op || ''",'
||' "threshold":'' || TO_CHAR(cond_rec.pr_val) || '', "failed":'' || CASE WHEN l_pr_failed THEN ''true'' ELSE ''false'' END || ''}'' ELSE ''null'' END || '', "event_score":'' || CASE WHEN cond_rec.es_val IS NOT NULL THEN ''{"actual":'' || NVL(TO_CHAR(l_actual_event_sc'
||'ore),''null'') || '', "operator":"'' || cond_rec.es_op || ''", "threshold":'' || TO_CHAR(cond_rec.es_val) || '', "failed":'' || CASE WHEN l_ev_failed THEN ''true'' ELSE ''false'' END || ''}'' ELSE ''null'' END || '', "expression":"'' || REPLACE(cond_rec.expr,''"'',''\"'')'
...
||'current_id), v_stay_date)); l_eval_copy := REPLACE(l_eval_copy, ''#'' || l_current_id || ''#'', NVL(l_attr_val, ''0'')); IF l_attr_counter > 0 THEN l_attr_json := l_attr_json || '',''; END IF; l_attr_json := l_attr_json || ''"'' || l_current_id || ''":'' || NVL('
||'l_attr_val, ''null''); l_pos := l_pos + 1; l_attr_counter := l_attr_counter + 1; END LOOP;',
'                    l_attr_json := l_attr_json || ''}'';',
'                    BEGIN',
...
'    v_app_id     NUMBER := NVL(TO_NUMBER(SYS_CONTEXT(''APEX$SESSION'',''APP_ID'')), 101); -- fall'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
'            BEGIN',
'                -- Extract the first table name from the formula (e.g., from #TABLE'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
'          v_message := ''Error dropping table or deleting template: '' || SQLERRM;',
'          dbg(v_message);',
'          append_result(rec.id, rec.hotel_id, rec.key, rec.name, rec.type, rec.active, rec.db_object_name, v_st'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28904151590267856)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
,p_object_name=>'FN_CLEAN_NUMBER'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28904356934267857)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28904548070267858)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28904719024267858)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28904910301267859)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28905138109267860)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
,p_object_name=>'NORMALIZE_JSON_1'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28905357619267861)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28905596226267862)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'FUNCTION'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28905954551267863)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28906113935267864)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28906343237267865)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28906469524267865)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28906618109267866)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28906803392267867)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28907030826267867)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28907228067267868)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28907414029267869)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28907686382267870)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PACKAGE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28907829534267870)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28908021244267871)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28908281421267872)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28908409734267873)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'DEFINE_DB_OBJECT_1'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28908608805267874)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'DELETE_TEMPLATES_1'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28908830479267874)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28909017771267875)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28909201802267876)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'JSON_TO_APEX_COLLECTION'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28909479612267877)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'JSON_TO_COLLECTION'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28909629928267877)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28909832435267878)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28910094297267879)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28910258009267880)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28910468842267880)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28910669427267881)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28910893179267882)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28911044919267883)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28911231639267883)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28911460322267884)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'VALIDATE_PROFILE_ROW'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28911676613267885)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28911883413267886)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28912048527267886)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'XX_LOCAL_LOAD_DATA_1'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28912229189267887)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'XX_LOCAL_LOAD_DATA_2'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28912439802267888)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'XX_LOCAL_LOAD_DATA_3'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28912652454267889)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'PROCEDURE'
,p_object_name=>'XX_LOCAL_LOAD_DATA_4'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28905781933267862)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TABLE'
,p_object_name=>'UR_16_OCT_CP_14_D1_MV'
);
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28912834790267889)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28913090496267890)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28913201902267891)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28913416063267892)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28913684227267892)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28913853710267893)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28914073016267894)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28914288980267895)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
...
wwv_flow_imp_shared.create_install_object(
 p_id=>wwv_flow_imp.id(28914411578267895)
,p_script_id=>wwv_flow_imp.id(28904048592267836)
,p_object_owner=>'#OWNER#'
,p_object_type=>'TYPE'
