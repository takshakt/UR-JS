...
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
'ICATE CHECK: stop if file already uploaded successfully',
'    -------------------------------------------------------------------',
'    SELECT COUNT(*)',
'      INTO l_existing_cnt',
'      FROM ur_interface_logs',
...
'  -- Just date DD/MM/YYYY',
'  '' WHEN REGEXP_LIKE(TRIM(p.''|| l_mapping(k).parser_col ||''), ''''^\d{2}/\d{2}/\d{4}$'''') ''||',
'  ''      THEN TO_DATE(TRIM(p.''|| l_mapping(k).parser_col ||''), ''''DD/MM/YYY'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
'    IF is_in_list(l_token, v_attributes, v_attr_count) THEN',
'      RETURN TRUE;',
'    ELSIF is_in_list(l_token, v_functions, v'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
...
'    -- OUT params',
'    p_status  := TRUE;',
'    p_message := ''Success: Upload completed for'))
);
wwv_flow_imp_shared.append_to_install_script(
 p_id=>wwv_flow_imp.id(28904048592267836)
,p_script_clob=>wwv_flow_string.join(wwv_flow_t_varchar2(
