...
function createCard(index, data) {
   // console.log('data:>>>>', data);
    ...
    // Get current row's PK_COL (could be "2025-11-01", "2025-W44", or "2025-11")
    const currentKey = row.PK_COL;
    let targetKey = null;

    if (currentKey) {
        // 1. Detect Week format (YYYY-WXX)
        ...
        // 3. Standard Date Offset (YYYY-MM-DD)
            ...
            targetKey = addDays(currentKey, offset);
        }

        // Find the value in originalData using the corrected targetKey
        const offsetValue = findValueInOriginalData(targetKey, columnRef);
...
Object.entries(offsetReplacements).forEach(([pattern, value]) => {
    ...
    if (value === null || value === undefined || value === '') {
        hasOffsetError = true;
        replacementValue = 'Calculation Issue'; // Use 0 to prevent math failure
    ...
    // Use split/join to replace ALL occurrences of the same pattern (e.g., Col{1} + Col{1})
    expr = expr.split(pattern).join(replacementValue);
});
...
if (hasOffsetError && Object.keys(offsetReplacements).length > 0 &&
    ...
    // 1. Same logic as before for data types
    if (dataTypeMap[col] === 'date' && value != null && value !== '' && value !== undefined) {
        value = `"${value}"`;
    } else if (value !== '' && value !== null && value !== undefined && !isNaN(value)) {
        value = parseFloat(value);
    } else if (value === null || value === '' || value === undefined) {
        value = 'Calculation Issue'; // consistent with our previous fixes
    } else {
        value = `"${value}"`;
    }

    // 2. THE CRITICAL FIX:
    // We use a Regex that ensures we DON'T match if there is a '{' immediately after the name.
    // This prevents [COL] from replacing part of [COL]{1}
    const escapedCol = col.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // Escape special chars
    
    // Replace bracketed version: [COL]
    expr = expr.replace(new RegExp(`\\[${escapedCol}\\](?!\\{)`, 'g'), value);
    
    // Replace unbracketed version: COL (using word boundaries and ensuring no { follows)
    expr = expr.replace(new RegExp(`\\b${escapedCol}\\b(?!\\{)`, 'g'), value);
});
